...
function PanelHeader({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 sm:p-5">
      <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-fuchsia-200/80 sm:text-xs sm:tracking-[0.25em]">{eyebrow}</p>
...
function FormInput({ label, value, placeholder, textarea, onChange }: { label: string; value: string; placeholder: string; textarea?: boolean; onChange: (value: string) => void }) {
  return (
    <label className="grid gap-2">
      ...
      {textarea ? (
        <textarea value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} rows={4} className="input min-h-24 resize-y text-base sm:text-sm" />
      ) : (
        <input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="input text-base sm:text-sm" />
      )}
    </label>
  );
}
...
function FormSelect({ label, value, options, onChange }: { label: string; value: string | number; options: Array<string | number | { label: string; value: string | number }>; onChange: (value: string) => void }) {
  return (
    <label className="grid gap-2">
      <span className="text-sm font-bold text-slate-100">{label}</span>
      <select value={value} onChange={(event) => onChange(event.target.value)} className="input cursor-pointer text-base sm:text-sm">
        {options.map((option) => {
...
export default function Home() {
  ...
  const moveSectionByOffset = (sectionId: SectionId, offset: number) => setForm((current) => {
    const currentIndex = current.sectionOrder.indexOf(sectionId);
    const nextIndex = currentIndex + offset;
    if (currentIndex < 0 || nextIndex < 0 || nextIndex >= current.sectionOrder.length) return current;
    const nextOrder = [...current.sectionOrder];
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#070713] text-white">
      ...
      <section className="relative mx-auto flex w-full max-w-7xl flex-col gap-5 px-3 py-4 sm:gap-7 sm:px-6 sm:py-6 lg:px-8 xl:px-10">
        <header className="rounded-3xl border border-white/10 bg-white/[0.04] p-4 shadow-2xl shadow-fuchsia-500/10 backdrop-blur sm:p-6">
        ...
        <div className="grid w-full min-w-0 gap-5 xl:grid-cols-[420px_minmax(0,1fr)]">
          <aside className="min-w-0 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] backdrop-blur">
            <div className="border-b border-white/10 p-4">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-1">{tabs.map((tab) => <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`rounded-2xl border p-3 text-left transition ${activeTab === tab.id ? 'border-fuchsia-300 bg-fuchsia-400/15 shadow-lg shadow-fuchsia-500/10' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.07]'}`}><div className="text-sm font-bold sm:text-base">{tab.label}</div><p className="hidden text-xs text-slate-400 sm:block">{tab.helper}</p></button>)}</div>
            </div>
            <div className="max-h-none overflow-auto p-4 sm:p-5 xl:max-h-[calc(100vh-220px)]">
              ...
              {activeTab === 'skills' ? <div className="grid gap-5"><PanelHeader eyebrow="Languages and tools" title="Pick your stack" description="Click skills to add or remove them. You can also edit the raw list." /><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><div className="mb-3 flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h3 className="font-black">Selected skills</h3><p className="text-xs text-slate-400">{selectedSkills.length} selected</p></div><button type="button" onClick={() => updateField('techStack', popularSkills.join(', '))} className="rounded-xl border border-cyan-300/30 bg-cyan-400/10 px-3 py-2 text-xs font-bold text-cyan-100">Use popular</button></div><textarea value={form.techStack} onChange={(event) => updateField('techStack', event.target.value)} rows={3} className="input min-h-24 resize-y text-base sm:text-sm" /></div>{skillGroups.map((group) => <div key={group.title} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><h3 className="mb-3 font-black">{group.title}</h3><div className="flex flex-wrap gap-2">{group.skills.map((skill) => { const selected = selectedSkills.includes(skill); return <button key={skill} type="button" onClick={() => toggleSkill(skill)} className={`rounded-xl border px-3 py-2 text-sm font-bold ${selected ? 'border-fuchsia-300 bg-fuchsia-400/15 text-fuchsia-50' : 'border-white/10 bg-white/[0.04] text-slate-300'}`}>{selected ? '✓ ' : ''}{skill}</button>; })}</div></div>)}</div> : null}
          ...
          <section className="grid min-w-0 gap-5 xl:sticky xl:top-6 xl:self-start">
            <div className="min-w-0 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] p-4 backdrop-blur sm:p-5">
              <div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h2 className="text-xl font-black">Preview</h2><p className="text-sm text-slate-300">Preview stays clean. Open section order only when you want to reorder.</p></div><div className="grid grid-cols-1 gap-2 sm:flex sm:flex-wrap"><button type="button" onClick={() => setShowSectionOrder((current) => !current)} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-4 py-3 font-bold text-slate-100 hover:bg-white/15">{showSectionOrder ? 'Hide Section Order' : 'Show Section Order'}</button><button type="button" onClick={copyMarkdown} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-fuchsia-500 px-4 py-3 font-bold text-white"><Copy size={18} />Copy</button><button type="button" onClick={downloadMarkdown} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-cyan-300/30 bg-cyan-400/10 px-4 py-3 font-bold text-cyan-100"><Download size={18} />Raw</button></div></div>
              {showSectionOrder ? <div className="mb-4 rounded-2xl border border-white/10 bg-black/20 p-4"><div className="mb-3 flex flex-col justify-between gap-2 sm:flex-row sm:items-center"><div><h3 className="font-black">Section Order</h3><p className="text-xs text-slate-400">Drag on desktop or use up/down buttons on mobile.</p></div><button type="button" onClick={() => setForm((current) => ({ ...current, sectionOrder: defaultSectionOrder }))} className="rounded-xl border border-white/10 bg-white/10 px-3 py-2 text-xs font-bold text-slate-200">Reset order</button></div><div className="grid gap-2">{form.sectionOrder.map((sectionId, index) => { const section = sectionCards[sectionId]; const dragging = draggingSection === sectionId; return <div key={sectionId} draggable onDragStart={(event) => { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', sectionId); setDraggingSection(sectionId); }} onDragEnd={() => setDraggingSection(null)} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'move'; }} onDrop={(event) => { event.preventDefault(); const draggedSection = event.dataTransfer.getData('text/plain') as SectionId; moveSection(draggedSection, sectionId); setDraggingSection(null); }} className={`group flex cursor-grab items-center gap-3 rounded-2xl border p-3 ${dragging ? 'border-fuchsia-300 bg-fuchsia-400/20 opacity-60' : 'border-white/10 bg-white/[0.04]'}`}><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/10 text-xs font-black text-slate-200">{index + 1}</div><GripVertical className="hidden shrink-0 text-slate-400 sm:block" size={18} /><div className="min-w-0 flex-1"><div className="font-bold text-white">{section.title}</div><p className="truncate text-xs text-slate-400">{section.description}</p></div><div className="flex shrink-0 gap-1 sm:hidden"><button type="button" onClick={() => moveSectionByOffset(sectionId, -1)} disabled={index === 0} className="rounded-lg border border-white/10 bg-white/10 px-2 py-1 text-xs font-bold disabled:opacity-30">↑</button><button type="button" onClick={() => moveSectionByOffset(sectionId, 1)} disabled={index === form.sectionOrder.length - 1} className="rounded-lg border border-white/10 bg-white/10 px-2 py-1 text-xs font-bold disabled:opacity-30">↓</button></div></div>; })}</div></div> : null}
              <article className="preview max-h-[65vh] min-w-0 max-w-full overflow-auto rounded-2xl border border-white/10 bg-[#0d0d1f] p-4 sm:max-h-[620px] sm:p-5"><ReactMarkdown rehypePlugins={[rehypeRaw]}>{markdown}</ReactMarkdown></article>
            </div>
            <details className="min-w-0 overflow-hidden rounded-3xl border border-white/10 bg-[#0d0d1f] p-4 sm:p-5"><summary className="cursor-pointer text-lg font-black">README.md</summary><p className="mt-2 text-sm text-slate-300">Copy or download this raw markdown into your GitHub profile repository.</p><div className="mt-4 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap"><button type="button" onClick={copyMarkdown} className="rounded-xl bg-fuchsia-500 px-3 py-2 text-xs font-bold text-white">Copy</button><button type="button" onClick={downloadMarkdown} className="rounded-xl border border-cyan-300/30 bg-cyan-400/10 px-3 py-2 text-xs font-bold text-cyan-100">Download Raw</button></div><pre className="mt-4 max-h-[50vh] max-w-full overflow-auto whitespace-pre-wrap break-words rounded-2xl border border-white/10 bg-black/40 p-4 text-xs leading-6 text-slate-200 sm:max-h-[420px] sm:text-sm"><code>{markdown}</code></pre></details>
          </section>
        </div>
      </section>
    </main>
